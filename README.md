...
Customer places order from checkout modal.
/api/create-order stores order in Supabase as pending.
If UPI_ID is configured, app opens a UPI payment link.
Payment status is updated when webhook/manual status endpoint is called.
